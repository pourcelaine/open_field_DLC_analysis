...
function [xy_data_to_plot] = read_DLC_output(subject_dir, subject_ID, timepoints, bodypart_to_track, LPfreq, downsample_number)

session_dir = fullfile(subject_dir, strcat(subject_ID, '_OF_', timepoints))

if isfile(fullfile(session_dir,  strcat(subject_ID, '_OF_', timepoints, '_DLC_xy_filtered_downsampled_clean.mat'))) == 1;
    xy_data_to_plot = load(fullfile(session_dir,  strcat(subject_ID, '_OF_', timepoints, '_DLC_xy_filtered_downsampled_clean.mat'))).DLC_xy_filtered_downsampled_clean;
    ans = 'yes'
elseif isfile(fullfile(session_dir,  strcat(subject_ID, '_OF_', timepoints, '_DLC_xy_filtered_downsampled.mat'))) == 1;
    xy_data_to_plot = load(fullfile(session_dir,  strcat(subject_ID, '_OF_', timepoints, '_DLC_xy_filtered_downsampled.mat'))).DLC_xy_filtered_downsampled;
    ans = 'no'
elseif isfile(fullfile(session_dir,  strcat(subject_ID, '_OF_', timepoints, '_DLC_xy_filtered_downsampled.mat'))) == 0;
    ans = 'no'
    ...
    xy_data_to_plot = DLC_xy_filtered_downsampled
end